...
const registerNotificationNamespace = (io, authMiddleware, options = {}) => {
    ...
    namespace.on(socket_1.SOCKET_EVENTS.CONNECTION, (socket) => {
        try {
            ...
            if (options.debug) {
                console.log(`[socket][notifications] user ${socketUser.userId} đã tham gia vào ${personalRoom} phòng`);
            }
            ...
            socket.on(socket_1.SOCKET_EVENTS.NOTIFICATION_SUBSCRIBE, (payload) => {
                ...
                if (options.debug) {
                    console.log(`[socket][notifications] user ${socketUser.userId} đã đăng ký vào ${payload.room} phòng`);
                }
            });
            socket.on(socket_1.SOCKET_EVENTS.NOTIFICATION_ACK, (payload) => {
                ...
                if (options.debug) {
                    console.log(`[socket][notifications] user ${socketUser.userId} đã xác nhận thông báo ${payload.notificationId}`);
                }
            });
