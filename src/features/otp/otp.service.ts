...
import { getOtpEmailTemplateConfig } from "../../utils/emailTemplates";
...
export default class OtpService {
   ...
   * @returns Mã OTP 6 số
   */
  private static generateSmartOtpCode(password: string): string {
  ...
  static async requestOtp(
    ...
    options?: {
      emailSubject?: string;
      emailTemplate?: (params: { userName: string; otpCode: string; appName?: string }) => string;
      appName?: string;
    }
  ) {
    if (!identifier) {
    ...
    if (channel === "email") {
      try {
        ...
        const defaultTemplate = getOtpEmailTemplateConfig(purpose, {
          userName,
          otpCode: code,
        ...
        const emailResult = await sendEmail(identifier, emailHtml, subject);
    ...
    return { ok: true as const, message: "OTP đã được tạo và gửi" };
  }
  ...
  static async verifyOtp(
    ...
    if (user?.otpMethod === OtpMethod.SMART_OTP && smartOtpPassword) {
      ...
      const isPasswordValid = await bcryptjs.compare(smartOtpPassword, user.smartOtpSecret);
      if (!isPasswordValid) {
        return {
          ok: false as const,
          status: 400,
      ...
      if (code !== smartOtpPassword) {
        ...
        if (code === generatedCode || code === prevCode || code === nextCode) {
          return { ok: true as const, message: "Xác minh Smart OTP thành công" };
        } else {
          return {
            ok: false as const,
            status: 400,
    ...
    const record = await OtpModel.findOne({ identifier, purpose, used: false }).sort({
      createdAt: -1,
    });
   ...
   */
  private static generateSmartOtpCodeFromTimeStep(password: string, timeStep: number): string {
    const hmac = crypto
      .createHmac("sha256", password)
      .update(String(timeStep))
      .digest("hex");
   ...
   */
  static async verifyOtpBeforeAction(
    identifier: string,
    code: string,
    purpose: string,
    smartOtpPassword?: string
