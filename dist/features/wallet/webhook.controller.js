...
const webhookReceiverController = async (req, res) => {
    try {
        ...
        const amount = parseFloat(body.transferAmount || body.amount || body.totalAmount || body.money || 0);
        ...
        if (orderMatch) {
            ...
            if (status === "completed" || status === "success") {
                const result = await payment_service_1.default.confirmBankTransferFromWebhook(orderId, transactionId || `WEBHOOK_${Date.now()}`, amount);
                if (result.ok) {
                    return res.status(200).json({
                        success: true,
